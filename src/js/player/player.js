...
export default class Player extends Sprite {
  ...
  update() {
    const cor = this.getCor();
    this.smallCircle = new Circle(cor.p[1].x, cor.p[1].y, cor.r0);
    if (this.immortalCount > 0) this.immortalCount -= 1;
    let posx = this.getX() + this.vel.x;
    let posy = this.getY() + this.vel.y;
    while (posx > this.constant.gameCor.width) posx -= this.constant.gameCor.width;
    while (posx < 0) posx += this.constant.gameCor.width;
    while (posy > this.constant.gameCor.height) posy -= this.constant.gameCor.height;
    while (posy < 0) posy += this.constant.gameCor.height;
  ...
  turn(angle) {
    ...
    while (delta < -Math.PI) delta += 2 * Math.PI;
    let tmp = delta;
    if (Math.abs(delta) > this.constant.turnAngle) tmp = delta > 0 ? this.constant.turnAngle : -this.constant.turnAngle;
    this.angleDelta = tmp;
  }
