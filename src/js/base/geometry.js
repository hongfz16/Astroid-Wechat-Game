...
class Circle {
  ...
  checkOneCollision(constant, cir) {
    const p0 = new Point(this.center.x, this.center.y);
    const p1 = new Point(cir.center.x, cir.center.y);
    if (p0.x - p1.x > constant.gameCor.width / 2) p1.x += constant.gameCor.width;
    else if (p1.x - p0.x > constant.gameCor.width / 2) p0.x += constant.gameCor.width;

    if (p0.y - p1.y > constant.gameCor.height / 2) p1.y += constant.gameCor.height;
    else if (p1.y - p0.y > constant.gameCor.height / 2) p0.y += constant.gameCor.height;
